...
const ROLE_SETS: [string, Roles][] = [
  ...
  ["protanDeutanLight", rolesProtanDeutanDark],
    ...
    describe(name, () => {
      test("defines every required role category", () => {
        for (const category of REQUIRED_ROLE_CATEGORIES) {
          assert.ok((roles as Record<string, unknown>)[category], `${name}: missing "${category}" category`);
        }
      });
...
type ThemeVariant = { name: string; displayName: string; type: "light" | "dark"; roles: Roles };
  ...
  for (const variant of THEME_VARIANTS) {
    describe(variant.displayName, () => {
      ...
      test("has the required top-level properties", () => {
        assert.ok(theme.name, "missing theme name");
        assert.ok(theme.displayName, "missing theme displayName");
        assert.ok(theme.type, "missing theme type");
        assert.ok(theme.colors, "missing colors object");
        assert.ok(theme.tokenColors, "missing tokenColors array");
        assert.ok(theme.semanticTokenColors, "missing semanticTokenColors object");
      });
      ...
      test("defines the critical editor colors", () => {
        for (const key of CRITICAL_COLORS) {
          assert.ok(theme.colors[key], `missing critical color: ${key}`);
        }
      });
      ...
      test("every tokenColors entry is well-formed", () => {
        theme.tokenColors.forEach((token, idx) => {
          assert.ok(token.scope, `tokenColors[${idx}] missing scope`);
          assert.ok(token.settings, `tokenColors[${idx}] missing settings`);
      ...
      test("every semanticTokenColors entry is valid", () => {
        for (const [key, value] of Object.entries(theme.semanticTokenColors)) {
          if (typeof value === "string") {
            usedColors.add(value);
            assert.ok(isValidColor(value), `semanticTokenColors["${key}"] has invalid color: ${value}`);
