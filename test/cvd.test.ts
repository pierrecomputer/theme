 ...
 * and is run as part of `npm test` (via the node:test runner). It fails the build
...
import { simulatedContrast, worstDeltaE, referenceCrossChecks } from "./helpers/cvd";
...
type RolePair = { tier: 1 | 2 | 3; label: string; a: Selector; b: Selector; group: string };
...
const DISTINGUISHABILITY_PAIRS: RolePair[] = [
  ...
  { tier: 1, group: "diff bg", label: "success(added) vs danger(deleted)", a: selectors.success, b: selectors.danger },
  // Diff TEXT tokens: inserted vs deleted, the semantic core of a review.
  { tier: 1, group: "diff text", label: "string(inserted) vs tag(deleted)", a: selectors.string, b: selectors.tag },
  // Merge conflict view: current(merge) vs incoming(info) tinted backgrounds.
  { tier: 1, group: "merge conflict", label: "merge vs info", a: selectors.merge, b: selectors.info },
  // Terminal pass/fail.
  { tier: 1, group: "terminal", label: "ansi.red vs ansi.green", a: selectors.ansiRed, b: selectors.ansiGreen },
  ...
  { tier: 2, group: "comment vs code", label: "comment vs string", a: selectors.comment, b: selectors.string },
  { tier: 2, group: "comment vs code", label: "comment vs keyword", a: selectors.comment, b: selectors.keyword },
  { tier: 2, group: "comment vs code", label: "comment vs variable", a: selectors.comment, b: selectors.variable },
  ...
  for (const { name, roles, cvds } of THEMES) {
    describe(`${name} [simulated as: ${cvds.join(", ")}]`, () => {
      ...
      describe("contrast", () => {
        ...
        test("report-only contrast (intrinsically-bright / brand colors)", (t) => {
      ...
      describe("distinguishability under simulation", () => {
        ...
        test("Tier 3 (advisory — reported, never fails)", (t) => {
          for (const p of DISTINGUISHABILITY_PAIRS.filter((p) => p.tier === 3)) {
            const { worst, worstCvd, worstConvention } = worstDeltaE(p.a(roles), p.b(roles), cvds);
            t.diagnostic(`[${p.group}] ${p.label} — ΔE ${worst.toFixed(1)} (${worstCvd}, ${worstConvention})`);
          }
        });
      });
    });
