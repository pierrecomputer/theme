...
//
//   • Protanopia  — missing L (long/red) cones  ┐ both confuse RED ↔ GREEN.
//   • Deuteranopia— missing M (medium/green)cones┘ The axis that can still be
//                   told apart is roughly BLUE ↔ ORANGE/YELLOW.
...
import { hexToRgb01, srgbToLinear, linearToSrgb } from "./srgb";
...
function channelToHex(v01: number): string {
  return Math.round(clamp01(v01) * 255).toString(16).padStart(2, "0");
}
...
export function cvdSelfChecks(): SelfCheckResult[] {
  ...
  for (const t of ["protan", "deutan", "tritan"] as CVDType[]) {
    ...
    results.push({ name: `severity-0 identity (${t})`, ok, detail: ok ? "input preserved" : "drifted" });
  }
  ...
  for (const t of ["protan", "deutan", "tritan"] as CVDType[]) {
    ...
    results.push({ name: `neutral axis preserved (${t})`, ok, detail: ok ? "grays stable" : "grays shifted" });
  }
  ...
  const red = "#ff2e3f", green = "#199f43", blue = "#009fff";
  const collapses = (name: string, t: CVDType, x: string, y: string) => {
    ...
    results.push({ name, ok, detail: `ΔE ${before.toFixed(1)} → ${after.toFixed(1)} under ${t}` });
  };
