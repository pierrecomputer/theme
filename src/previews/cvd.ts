...
// proof matches intuition. Run with `npm run preview`.
import {
  protanDeutanLight, protanDeutanDark, tritanopiaLight, tritanopiaDark,
  type Roles,
} from "../roles";
import { simulateCVD, type CVDType } from "../color";

type View = { title: string; roles: Roles; cvd: CVDType; type: "light" | "dark" };
...
const VIEWS: View[] = [
  // Light
  { title: "Pierre Light Protanopia & Deuteranopia", roles: protanDeutanLight, cvd: "protan", type: "light" },
  { title: "Pierre Light Protanopia & Deuteranopia", roles: protanDeutanLight, cvd: "deutan", type: "light" },
  { title: "Pierre Light Tritanopia", roles: tritanopiaLight, cvd: "tritan", type: "light" },
  // Dark
  { title: "Pierre Dark Protanopia & Deuteranopia", roles: protanDeutanDark, cvd: "protan", type: "dark" },
  { title: "Pierre Dark Protanopia & Deuteranopia", roles: protanDeutanDark, cvd: "deutan", type: "dark" },
  { title: "Pierre Dark Tritanopia", roles: tritanopiaDark, cvd: "tritan", type: "dark" },
];
...
function simulateRoles(r: Roles, cvd: CVDType): Roles {
  const walk = (o: any): any => {
    if (typeof o === "string") return /^#[0-9a-fA-F]{6}$/.test(o) ? simulateCVD(o, cvd) : o;
    const out: any = {};
...
function mix(fg: string, bg: string, a: number): string {
  ...
  const [r1, g1, b1] = h(fg), [r2, g2, b2] = h(bg);
...
function mock(r: Roles): string {
  const ed = r.bg.editor, win = r.bg.window;
